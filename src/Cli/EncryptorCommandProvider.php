...
final class EncryptorCommandProvider implements CommandProviderInterface
{
    public function register(CommandRegistryInterface $registry): void
    {
        ...
        $registry->register(Command::define(
            name: 'encryptor:password:generate',
            description: 'Сгенерировать пароль (url-safe по умолчанию)',
            signature: [
                new OptionDefinition(
                    name: 'length',
                    short: 'l',
                    description: 'Длина пароля в байтах (до кодирования)',
                    required: false,
                    default: 24,
                    type: 'int',
                ),
                new OptionDefinition(
                    name: 'format',
                    short: 'f',
                    description: 'Формат пароля: base64url, hex, base64',
                    required: false,
                    default: 'base64url',
                    type: 'string',
                ),
            ],
            handler: GeneratePasswordHandler::class,
        ));
    }
}
